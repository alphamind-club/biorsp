...
def make_metric_matrix(
    data: Dict[str, pd.DataFrame], metric: str = "CRA_W1"
) -> pd.DataFrame:
...
def venn_top_genes(
    ...
    for df in data.values():
        if "CRA_W1" not in df.columns:
            continue
        top_idx = df.sort_values("CRA_W1", ascending=False)
        if top_n is not None:
...
def main():
    ...
    parser.add_argument(
        "--metric",
        default="CRA_W1",
        help="Metric column to use for heatmap/bar/venn (default: CRA_W1)",
    )
    parser.add_argument(
        "--heatmap-top",
        type=int,
